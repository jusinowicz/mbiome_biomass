...
####Print a breakdown by Ecosystem
summary1 = summarise(group_by(dfa_B, Ecosystem), Total = n()) 

#pdf(file="./data/total_ecosystems_bar1.pdf", height=8, width=8, onefile=TRUE, family='Helvetica', pointsize=16)
#jpeg(file="./data/total_ecosystems_bar1.jpg", width = 8, height = 6, units = "in", res = 300)
ggplot(summary1, aes(x = Total, y = Ecosystem, fill = Ecosystem)) +
  geom_bar(stat = "identity") +
  labs(title = "Total Sites by Ecosystem",
       y = "Ecosystem",
       x = "Total plots") +
  theme_minimal() +
  theme(legend.position = "none", 
    axis.text.x = element_text(size = 16),
    axis.text.y = element_text(size = 16),  # Adjust size for y-axis text
    axis.title.x = element_text(size = 20),  # Adjust size for x-axis title
    axis.title.y = element_text(size = 20)   # Adjust size for y-axis title
  )+
...
readable_names  = c("Precip. Wettest","clay", "M Coldest T","Isothermality","Prec Wettest Q", 
  "Prec Dryest Q","M T Driest","Min T", "M An Temp", "bulk dens.","wv1500", "Precip Dry",
"nitrogen","wv0033","ph","Season T", "Precip Cold", "CEC", "An Precip","Coarse Frag", "sand",
"T An Range", "M Diurn Rang", "Precip Hot", "M T Hot", "S Organ C", "Season Pr","silt","M T Wet",
"Max T")

#pdf(file=fig.name, height=8, width=8, onefile=TRUE, family='Helvetica', pointsize=16)
jpeg(file="./data/variable_importance2_single.jpg", width = 8, height = 6, units = "in", res = 300)

ggplot(import_ranks, aes(y = factor(IncNodePurity), x = IncNodePurity)) +
  geom_point() + 
  geom_errorbar(aes(xmin = IncNodePurity - sd, xmax = IncNodePurity + sd), width = 0.2) +
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  scale_y_discrete(labels = readable_names )+
  labs(y = 'Variable', x = 'Mean Rank', title = 'Mean Variable Importance ') +
  #theme_minimal()+
  theme(legend.position = "none", 
    axis.text.x = element_text(size = 14),
    axis.text.y = element_text(size = 14),  # Adjust size for y-axis text
    axis.title.x = element_text(size = 18),  # Adjust size for x-axis title
    axis.title.y = element_text(size = 18)   # Adjust size for y-axis title
  )
dev.off()
...
#Current version after data fix
model_full2a = rma.mv(yi,vi, mods = ~bdodmean+MinT+MTempC+MTempD+PrDQ+PrD+wv1500mean+
                                    PrW+Iso+PrWQ+MDR+claymean+AMT+
                                    nitrogenmean,
                                    random =   ~ 1 | Study/studyGroup, 
                                    data= dfa_ME, method = "ML")

# #This is the more hypothesis-based version:
# model_full2b = rma.mv(yi,vi, mods = ~AMT + PrWQ + PrDQ + MinT + SeaPr +
#                                     bdodmean + wv1500mean + claymean + cfvomean+
#                                     nitrogenmean + phh2omean + cecmean + sandmean,
#                                     random =   ~ 1 | Study/studyGroup, 
#                                     data= dfa_ME, method = "ML")
...
#####>>>>>>>>>>>>>>>Comment/uncomment here as needed <<<<<<<<<<<<<<<<<<<<
#model_dredge = dredge(model_full2b, trace=2)
#save(file="./data/dredged_RE_model2b.var", model_dredge)
#load(file="./data/dredged_RE_model2b.var")
load(file="./data/dredged_RE_model2a.var")
...
#####To export summaries into csv: 
# fp = "./data/best_re_mod2a.csv"                                                                                                
# sink(file = fp)                                                                                                              
# summary(top_model)                                                                                                           
# sink()

####Plot variables and estimates as bar plot
readable_names  = c("Precip. Wettest","clay", "M Coldest T","Isothermality","Prec Wettest Q", 
  "Prec Dryest Q","M T Driest","Min T", "M An Temp", "bulk dens.","wv1500", "Precip Dry",
"nitrogen","wv0033","ph","Season T", "Precip Cold", "CEC", "An Precip","Coarse Frag", "sand",
"T An Range", "M Diurn Rang", "Precip Hot", "M T Hot", "S Organ C", "Season Pr","silt","M T Wet",
...
#Get the coefficient summaries from the top model
sum1 = summary(top_model)
sum1 = data.frame(variable = rownames(sum1_temp$b),estimate = sum1_temp$b, se = sum1_temp$se)

#pdf(file=fig.name, height=8, width=8, onefile=TRUE, family='Helvetica', pointsize=16)
jpeg(file="./data/variables_lme_top2a.jpg", width = 8, height = 6, units = "in", res = 300)

ggplot(, aes(y = , x = IncNodePurity)) +
  geom_bar(stat = "identity") + 
  geom_errorbar(aes(xmin = IncNodePurity - sd, xmax = IncNodePurity + sd), width = 0.2) +
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  scale_y_discrete(labels = readable_names )+
  labs(y = 'Variable', x = 'Effect Size', title = '') +
  #theme_minimal()+
  theme(legend.position = "none", 
    axis.text.x = element_text(size = 14),
    axis.text.y = element_text(size = 14),  # Adjust size for y-axis text
